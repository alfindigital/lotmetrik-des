...
::selection{{background:rgba(20,184,166,.22);color:var(--navy)}}
[data-theme="terminal"] ::selection{{background:rgba(45,212,191,.35);color:#F5F7FA}}
...
.actions{{display:flex;justify-content:center;align-items:center;margin:4px 0 16px}}
...
@media(max-width:560px){{
  .stats-line{{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}}
  .stat{{padding:10px 10px}}
  .stat .sv{{font-size:1.25rem}}
  .stat .sl{{font-size:11px}}
}}
@media(max-width:380px){{
  .stat{{padding:8px 8px}}
  .stat .sv{{font-size:1.15rem}}
